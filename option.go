...
func setField(obj STIXObject, field string, value interface{}, kind reflect.Kind) error {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to apply %s option", field)
	}

	if f.Kind() != kind {
		return errors.Errorf("%s field is not a %s type", kind, field)
	}
...
func reflectValue(obj STIXObject) (reflect.Value, error) {
	...
	if val.Kind() != reflect.Ptr {
		return reflect.Value{}, errors.Errorf("object must be a pointer type")
	}

	val = val.Elem()

	if val.Kind() != reflect.Struct {
		return reflect.Value{}, errors.Errorf("object has to be a pointer to struct")
	}
...
func getFieldFromBaseStruct(obj reflect.Value, field string) (reflect.Value, error) {
	...
	for _, baseString := range baseStructs {
		...
		if b.Kind() != reflect.Struct {
			return reflect.Value{}, errors.Errorf("%s is not of struct type", baseString)
		}
	...
	return reflect.Value{}, errors.Errorf("%s not found", field)
}
