...
func mappingByType() map[STIXType]interface{} {
	return map[STIXType]interface{}{
		TypeAutonomousSystem:    &AutonomousSystem{},
		TypeArtifact:            &Artifact{},
		TypeAttackPattern:       &AttackPattern{},
		TypeCampaign:            &Campaign{},
		TypeCourseOfAction:      &CourseOfAction{},
		TypeDirectory:           &Directory{},
		TypeDomainName:          &DomainName{},
		TypeEmailAddress:        &EmailAddress{},
		TypeEmailMessage:        &EmailMessage{},
		TypeExtensionDefinition: &ExtensionDefinition{},
		TypeFile:                &File{},
		TypeGrouping:            &Grouping{},
		TypeIPv4Addr:            &IPv4Address{},
		TypeIPv6Addr:            &IPv6Address{},
		TypeIdentity:            &Identity{},
		TypeIndicator:           &Indicator{},
		TypeInfrastructure:      &Infrastructure{},
		TypeIntrusionSet:        &IntrusionSet{},
		TypeLanguageContent:     &LanguageContent{},
...
func objectFromType(collection *Collection, typ STIXType) (interface{}, bool) {
	mapping := mappingByType()

	obj, found := mapping[typ]
	if !found {
		if collection.dropCustom {
			return nil, true
		}

		return &CustomObject{}, false
	}
...
func processObjects(collection *Collection, objects []json.RawMessage) error {
	...
	for _, data := range objects {
		...
		obj, shouldSkip := objectFromType(collection, STIXType(typ.(string)))
		if shouldSkip {
			continue
		}
		...
		if err != nil {
			return errors.Wrap(err, "bad json data")
		}
...
func valueToMap(obj interface{}, m map[string]interface{}) error {
	...
	// Not a struct, nothing else to do.
	if typ.Kind() != reflect.Struct {
		return errors.Errorf("%s is an invalid type", typ.Name())
	}
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		default:
			...
			if fv.Kind() != reflect.Bool && fv.IsZero() {
				...
				if !skip {
					return errors.Errorf("%s is empty but is required", f.Name)
				}
				continue
			}
