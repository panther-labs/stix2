...
func (i *Infrastructure) AddConsistsOf(id Identifier, opts ...STIXOption) (*Relationship, error) {
	...
	return NewRelationship(RelationshipTypeConsistsOf, i.ID, id, opts...)
}

// AddControls describes that this infrastructure controls some other
// infrastructure or a malware instance (or family).
func (i *Infrastructure) AddControls(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForTypes(TypeInfrastructure, TypeMalware) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeControls, i.ID, id, opts...)
}
...
func (i *Infrastructure) AddCommunicatesWith(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForTypes(TypeInfrastructure, TypeIPv4Addr, TypeIPv6Addr, TypeDomainName, TypeURL) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeCommunicatesWith, i.ID, id, opts...)
}

// AddDelivers describes that this infrastructure controls some other
// infrastructure or a malware instance (or family).
func (i *Infrastructure) AddDelivers(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForType(TypeMalware) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeDelivers, i.ID, id, opts...)
}

// AddHas describes that this specific Infrastructure has this specific
// Vulnerability.
func (i *Infrastructure) AddHas(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForType(TypeVulnerability) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeHas, i.ID, id, opts...)
}

// AddHosts describes that this infrastructure has a tool running on it or is
// used to passively host the tool / malware.
func (i *Infrastructure) AddHosts(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForTypes(TypeMalware, TypeTool) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeHosts, i.ID, id, opts...)
}

// AddLocatedAt describes that the infrastructure originates from the related
// location.
func (i *Infrastructure) AddLocatedAt(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForType(TypeLocation) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeLocatedAt, i.ID, id, opts...)
}

// AddUses describes that this infrastructure uses this other infrastructure to
// achieve its objectives.
func (i *Infrastructure) AddUses(id Identifier, opts ...STIXOption) (*Relationship, error) {
	if !IsValidIdentifier(id) || !id.ForType(TypeInfrastructure) {
		return nil, ErrInvalidParameter
	}
	return NewRelationship(RelationshipTypeUses, i.ID, id, opts...)
}
